...
def eval_auc(dataset='OTB2015', tracker_reg='S*', start=0, end=1e6):
    ...
    trackers = glob.glob(os.path.join('test', dataset, 'SiamRPN_AlexNet_TinyTLP_V2'))
    #print(trackers)
    trackers = trackers[start:min(end, len(trackers))]
    #print(trackers)

    n_seq = len(seqs)
    #print("nseq = " + str(n_seq))
    thresholds_overlap = np.arange(0, 1.05, 0.05)
    # thresholds_error = np.arange(0, 51, 1)
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print('python eval_TinyTLP_V2.py TinyTLP_V2 SiamRPN 0 10')
        exit()
    print(sys.argv[1])
    print(sys.argv[2])
    dataset = sys.argv[1]
