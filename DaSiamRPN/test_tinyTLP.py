...
from net import SiamRPNotb, SiamRPNvot
from run_SiamRPN import SiamRPN_init, SiamRPN_track
from utils import rect_2_cxy_wh, cxy_wh_2_rect

parser = argparse.ArgumentParser(description='PyTorch SiamRPN TLP Test')
parser.add_argument('--dataset', dest='dataset', default='TinyTLP_V2', help='datasets')
...
def track_video(model, video):
    ...
    # save result
    video_path = join('test', args.dataset, 'SiamRPN_AlexNet_TinyTLP_V2')
...
def load_dataset(dataset):
    ...
    if not exists(base_path):
        print("Please download TLP dataset into `data` folder!")
        exit()
...
def main():
    ...
    net.load_state_dict(torch.load(join(realpath(dirname(__file__)), 'SiamRPNVOT.model'))) #changed OTB to VOT
    net.eval().cuda()
